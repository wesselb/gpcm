...
def test_merge_common_factors():
    assert set(_merge_common_factors(Factor('t1', 1),
                                     Factor('t2', 2),
                                     Factor('t3', 3),
                                     Factor('t2', 4),
                                     Factor('t1', 5))) == \
           {Factor('t1', 6), Factor('t2', 6), Factor('t3', 3)}
...
def test_term_substitute(t1, t2):
    assert Term(2,
                Factor('t1', 1),
                Factor('t2', 2)).substitute('t2', t1 - t2) == \
           2*t1**3 - 4*t1**2*t2 + 2*t1*t2**2
...
def test_term_equality(f):
    assert f(Term(2, Factor('t1', 1), Factor('t2', 2))) == \
           f(Term(2, Factor('t2', 2), Factor('t1', 1)))
    assert f(Term(2, Factor('t1', 1), Factor('t2', 2))) != \
           f(Term(2, Factor('t1', 2), Factor('t2', 2)))
    assert f(Term(1, Factor('t1', 1), Factor('t2', 2))) != \
           f(Term(2, Factor('t1', 1), Factor('t2', 2)))


def test_term_addition():
    assert Term(1, Factor('t', 2)) + Term(2, Factor('t', 2)) == \
           Term(3, Factor('t', 2))
...
def test_term_multiplication():
    assert Term(1, Factor('t1', 1))*Term(2, Factor('t2', 2)) == \
           Term(2, Factor('t1', 1), Factor('t2', 2))
...
def test_merge_common_terms():
    assert set(_merge_common_terms(Term(1, Factor('t1', 1)),
                                   Term(2, Factor('t2', 1)),
                                   Term(3, Factor('t3', 1)),
                                   Term(4, Factor('t2', 1)),
                                   Term(5, Factor('t1', 1)))) == \
...
def test_poly_collect_for(t1, t2, t3):
    assert (t1**2*t2*t3**3 + 2*t1**2).collect_for(Factor('t1', 2)) == \
           2 + t2*t3**3
...
def test_poly_substitute(t1, t2, t3):
    assert (2*t3**2).substitute('t3', t1 - t2) == 2*t1**2 - 4*t1*t2 + 2*t2**2
...
def test_poly_constructor():
    assert Poly(Term(1, Factor('t1', 1)),
                Term(2, Factor('t1', 1)),
                Term(2, Factor('t2', 2))) == \
           Poly(Term(3, Factor('t1', 1)),
                Term(2, Factor('t2', 2)))
...
def test_poly_addition(t1, t2, t3):
    assert Poly(Term(1, Factor('t1', 1))) + Poly(Term(1, Factor('t2', 1))) == \
           Poly(Term(1, Factor('t1', 1)), Term(1, Factor('t2', 1)))
    assert t1 + 0 is t1
    assert 0 + t1 is t1
...
def test_poly_multiplication(t1, t2, t3):
    assert Poly(Term(1, Factor('t1', 1)))*Poly(Term(2, Factor('t2', 1))) == \
           Poly(Term(2, Factor('t1', 1), Factor('t2', 1)))
    assert t1*1 is t1
    assert 1*t1 is t1
...
def test_exppoly_substitute(t1, t2, t3):
    assert ExpPoly(2, -t3**2).substitute('t3', t1 - t2) == \
           ExpPoly(2, -t1**2 + 2*t1*t2 - t2**2)
...
def test_exppoly_str(t1, t2):
    ep = ExpPoly(2, t1 + t2)
    assert str(ep) in {'2 * exp(1 * t1^1 + 1 * t2^1)',
                       '2 * exp(1 * t2^1 + 1 * t1^1)'}
...
def test_exppoly_case3(ep2):
    ref = np.array([[1627.297351, 393.5943995],
                    [393.5943995, 1627.297351]])
    res = ep2.integrate_box(('t1', -np.inf, np.inf),
                            ('t2', -np.inf, np.inf),
                            t3=np.eye(2))
...
def test_exppoly_case4(ep2):
    ...
    res = ep2.integrate_box(('t1', -np.inf, 0),
                            ('t2', -np.inf, 0),
                            t3=np.eye(2))
    approx(res, ref, decimal=6)
